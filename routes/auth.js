...
authRouter.post("/login", async (req, res, next) => {
    ...
    try {
        ...
        !user && res.status(401).json({ "message": messages.NOT_AUTHORIZED, status: false })
        const hashedPassword = CryptoJS.AES.decrypt(OriginalPassword, AUTH_PASSWORD_SECRET)
        ...
        res.status(200).json({ "message": "login success", user: rest, token: accessToken });
